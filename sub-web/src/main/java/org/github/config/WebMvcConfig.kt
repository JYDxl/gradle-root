...
class WebMvcConfig: WebMvcConfigurer {
  ...
  fun returnableValueHandler(adapter: RequestMappingHandlerAdapter) = ReturnableValueHandler().apply {
    adapter.returnValueHandlers = builder<HandlerMethodReturnValueHandler>().add(ReturnableValueHandlerKotlin).addAll(adapter.returnValueHandlers ?: emptyList()).add(this).build()
  }
}
