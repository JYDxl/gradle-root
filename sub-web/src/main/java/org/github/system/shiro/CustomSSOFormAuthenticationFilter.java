...
public class CustomSSOFormAuthenticationFilter extends CustomFormAuthenticationFilter {
    ...
    protected boolean onLoginSuccess(AuthenticationToken token, Subject subject, ServletRequest request, ServletResponse response) throws Exception {
        ...
        val currentSession = subject.getSession(false);
        if (currentSession != null) {
            ...
            val removeList = activeSessions.stream().filter(session -> currentUser(currentSession, session)).collect(toImmutableList());
            removeList.forEach(sessionDAO::delete);
        }
        return false;
    }

    private boolean currentUser(Session currentSession, Session session) {
        val currentUser = (UserInfoDTO) currentSession.getAttribute("user");
        val user = (UserInfoDTO) session.getAttribute("user");
        return !Objects.equals(currentSession.getId(), session.getId()) && Objects.equals(currentUser.getId(), user.getId());
    }
}
