...
class SsoServerController {
  ...
  private fun configSso(sso: SaSsoConfig) {
    // 配置：未登录时返回的View
    sso.setNotLoginView {JSONBase.of(UNAUTHORIZED, null)}
    // 配置：登录处理函数
  ...
  private fun getRedirectUrl(redirect: String, mode: String): JSONBase {
    // 未登录
    if (!isLogin()) return JSONBase.of(UNAUTHORIZED, null)
    // 已登录
    return if (MODE_SIMPLE == mode) {
      // 模式一
      checkRedirectUrl(decoderUrl(redirect))
      JSONData.of(redirect)
    } else {
      // 模式二或模式三
      val redirectUrl = buildRedirectUrl(getLoginId(), redirect)!!
      JSONData.of(redirectUrl)
    }
  }
