...
class SsoServerController {
    ...
    private fun configSso(sso: SaSsoConfig) {
        ...
        // 配置：登录处理函数
        sso.setDoLoginHandle { _: String?, _: String? -> centerService.login(objectMapper.readValue(req!!.inputStream, LoginBo::class.java)) }
    }
    ...
    private fun login(bo: LoginBo) = centerService.login(bo)
