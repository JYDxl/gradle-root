...
class SysMenuNameJ2Cache: AbstractJ2Cache<SysMenuEntity?, String?>() {
  ...
  override fun load(keys: Collection<String>): Map<String, SysMenuEntity?> {
    ...
    query.`in`(keys.isNotEmpty(), SysMenuEntity::id, keys.map {it.toLong()})
    query.select(SysMenuEntity::id, SysMenuEntity::name, SysMenuEntity::enabled)
    val list = query.ktList()
    return list.associateBy {it.id.toString()}
  }
}
