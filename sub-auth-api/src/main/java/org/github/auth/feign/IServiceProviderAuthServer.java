...
public interface IServiceProviderAuthServer {
    ...
    JSONDataReturn<UserDTO> user(@RequestParam String token, @PathVariable String username);

    @GetMapping("/security/auth/{userId}")
    JSONDataReturn<List<AuthorInfo>> auth(@RequestParam String token, @PathVariable String userId);
}
