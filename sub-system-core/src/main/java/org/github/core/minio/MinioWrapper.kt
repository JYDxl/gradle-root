...
import io.minio.http.Method.GET
...
class MinioWrapper(private val minio: MinioClient) {
  ...
  fun getUrl(bo: MinioQueryBo): String {
    val args = GetPresignedObjectUrlArgs.builder().method(GET).bucket(bo.bucket).`object`(bo.fullName).build()!!
    return try {
      minio.getPresignedObjectUrl(args)
    } catch (e: Exception) {
      throw ServerException("获取地址失败", e)
    }
  }
  ...
  fun upload(bo: MinioUploadBo) {
    ...
    val size = if (objSize == -1L) Size(-1L, parse("8MB")) else Size(bo.objSize, -1L)
  ...
  private class Size(val itemSize: Long = 0, val partSize: Long = 0)
}
