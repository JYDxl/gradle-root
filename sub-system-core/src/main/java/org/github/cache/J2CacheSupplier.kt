...
interface J2CacheSupplier<E, R>: CacheSupplier<E, R> {
  ...
  @Suppress("UNCHECKED_CAST")
  override fun get(key: Any?): R? = if (key == null) null else mapper.apply(channel.get(region, key.toString(), {apply(key.toString())}).value as E?)

  @Suppress("UNCHECKED_CAST")
  override fun getSome(keys: Collection<String>): Map<String, R?> = channel.get(region, keys, this::apply).mapValues {mapper.apply(it.value.value as E?)}

  @Suppress("UNCHECKED_CAST")
  override fun getAll(filter: Predicate<E?>): List<Pair<String, R>> = channel.get(region, channel.keys(region)).filterValues {filter.test(it.value as E?)}.mapTo(arrayListOf()) {Pair<String, R>(it.key, mapper.apply(it.value.value as E))}
