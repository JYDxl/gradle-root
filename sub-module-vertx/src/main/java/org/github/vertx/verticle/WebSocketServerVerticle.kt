...
import io.netty.handler.codec.http.websocketx.WebSocketCloseStatus.INTERNAL_SERVER_ERROR
...
import org.github.vertx.ws.ServerWebSocketProxy
...
class WebSocketServerVerticle(private val port: Int = 8800): CoroutineVerticle() {
  ...
  private suspend fun initWebSocketServer() {
    val channel: ReceiveChannel<ServerWebSocket> = httpServer.webSocketStream().toReceiveChannel(vertx)
    launch {
      for (ws in channel) {
        //握手
